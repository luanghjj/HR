...
async function initApp() {
  ...
  try {
    ...
    console.error('[initApp] Fehler beim Laden:', err);
    state.initError = err?.message || 'Verbindungsfehler';
    state.monate = [];
    state.currentMonat = 'Mär 2026';
  ...
  if (state.initError) {
    main.appendChild(el('div', {
      style: {
        ...
        color: '#b91c1c', padding: '8px 16px', fontSize: '12px',
        display: 'flex', alignItems: 'center', gap: '8px'
      }
    },
      el('span', {}, '⚠️'),
      el('span', {}, `Supabase-Fehler: ${state.initError} — App läuft im Offline-Modus`),
      el('button', {
        style: { marginLeft: 'auto', fontSize: '11px', background: '#b91c1c', color: '#fff', border: 'none', borderRadius: '4px', padding: '3px 8px', cursor: 'pointer' },
        onClick: () => { state.initError = null; state.loading = true; renderApp(); initApp(); }
      }, '↺ Retry')
    ));
  }
...
function renderAbrechnung() {
  ...
  const searchGroup = el('div', {
    style: { display: 'flex', alignItems: 'center', gap: '0', border: '1px solid var(--bd)', borderRadius: '6px', overflow: 'hidden', background: 'var(--sf)' }
  });
  ...
  searchGroup.appendChild(el('button', {
    style: {
      border: 'none', borderLeft: '1px solid var(--bd)', background: 'var(--ac)', color: '#fff',
      fontSize: '9px', fontWeight: 600, padding: '5px 10px', cursor: 'pointer', fontFamily: 'var(--font)',
      whiteSpace: 'nowrap'
    },
    onClick: () => { document.getElementById('abr-search-input')?.focus(); }
  }, '\ud83d\udd0d Suchen'));
  ...
  if (state.abrSearch) {
    ...
    filtered = filtered.filter(d =>
      ...
      d.betrieb.toLowerCase().includes(q) ||
      (d.ue_bank || '').toLowerCase().includes(q) ||
      (d.notiz || '').toLowerCase().includes(q)
    );
  }
  ...
  const sortCols = [
    { key: 'persNr',   label: 'PERS.NR',    sortable: true,  type: 'num' },
    { key: 'name',     label: 'NAME',        sortable: true,  type: 'str' },
    { key: 'betrieb',  label: 'BETRIEB',     sortable: true,  type: 'str' },
    { key: 'gehalt',   label: 'GEHALT',      sortable: true,  type: 'num' },
    { key: 'brutto',   label: 'GES.-BRUTTO', sortable: true,  type: 'num' },
    { key: 'netto',    label: 'NETTO',       sortable: true,  type: 'num' },
    { key: 'ueberweisung', label: '\u00dcBERW.', sortable: true, type: 'num' },
    { key: 'ue_status', label: '\u00dc-STATUS', sortable: true, type: 'str' },
    { key: 'bar_tg',   label: 'BAR/TG',     sortable: true,  type: 'num' },
    { key: 'bar_status', label: 'B-STATUS',  sortable: true,  type: 'str' },
    { key: 'summe',    label: 'SUMME',       sortable: true,  type: 'num' },
    { key: 'notiz',    label: 'NOTIZ',       sortable: false, type: 'str' },
  ];
  ...
  filtered.sort((a, b) => {
    let va, vb;
    if (sc.col === 'summe') {
      va = a.ueberweisung + a.bar_tg;
      vb = b.ueberweisung + b.bar_tg;
    } else {
      va = a[sc.col]; vb = b[sc.col];
    }
    if (va == null) va = '';
    if (vb == null) vb = '';
    let cmp;
